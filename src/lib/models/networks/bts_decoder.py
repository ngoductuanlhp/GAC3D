...
def bn_init_as_tf(m):
    if isinstance(m, nn.BatchNorm2d):
        # These two lines enable using stats (moving mean and var) loaded from pretrained model
        m.track_running_stats = True
        # or zero mean and variance of one if the batch norm layer has no pretrained values
        m.eval()
        m.affine = True
...
class atrous_conv(nn.Sequential):
    def __init__(self, in_channels, out_channels, dilation, apply_bn_first=True):
        ...
        if apply_bn_first:
            self.atrous_conv.add_module('first_bn', nn.BatchNorm2d(
                in_channels, momentum=0.01, affine=True, track_running_stats=True, eps=1.1e-5))

        self.atrous_conv.add_module('aconv_sequence', nn.Sequential(nn.ReLU(),
                                                                    nn.Conv2d(
                                                                        in_channels=in_channels, out_channels=out_channels * 2, bias=False, kernel_size=1, stride=1, padding=0),
                                                                    nn.BatchNorm2d(
...
class local_planar_guidance(nn.Module):
    def __init__(self, upratio):
        ...
        self.u = torch.arange(self.upratio).reshape(
            [1, 1, self.upratio]).float()
        self.v = torch.arange(int(self.upratio)).reshape(
            [1, self.upratio, 1]).float()
        self.upratio = float(upratio)

    def forward(self, plane_eq, focal=None):
        ...
        u = self.u.repeat(plane_eq.size(0), plane_eq.size(
            2) * int(self.upratio), plane_eq.size(3)).cuda()
        u = (u - (self.upratio - 1) * 0.5) / self.upratio

        v = self.v.repeat(plane_eq.size(0), plane_eq.size(
            2), plane_eq.size(3) * int(self.upratio)).cuda()
...
class BTS_Decoder(nn.Module):
    def __init__(self, max_depth, feat_out_channels, num_features=512):
        ...
        self.daspp_6 = atrous_conv(
            num_features // 2 + num_features // 4 + feat_out_channels[2], num_features // 4, 6)
        ...
        self.reduc1x1 = reduction_1x1(
            num_features // 16, num_features // 32, self.max_depth, is_final=True)
    ...
    def forward(self, features):
        ...
        concat1 = torch.cat(
            [upconv1, reduc1x1, depth_2x2_scaled, depth_4x4_scaled, depth_8x8_scaled], dim=1)
        iconv1 = self.conv1(concat1)
        depth_prob = self.get_depth(iconv1)
        final_depth = self.max_depth * depth_prob
        # if self.params.dataset == 'kitti':
        #     final_depth = final_depth * \
        #         focal.view(-1, 1, 1, 1).float() / 715.0873

        return final_depth, depth_prob
...
#             self.feat_names = ['relu0', 'pool0',
#                                'transition1', 'transition2', 'norm5']
