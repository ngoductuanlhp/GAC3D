...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                    PacConv2d(channels[self.first_level], head_conv,
                              kernel_size=3, padding=1, bias=True),
                    nn.ReLU(inplace=True),
    ...
    def forward(self, x):
        ...
        '''Depth head'''
        depth_prob = self.depth_head(y[-1])
        depth_map = F.interpolate(
            depth_prob, scale_factor=4, mode="bilinear", align_corners=True)
        final_depth = self.max_depth * depth_map
        z['depth_est'] = final_depth

        inp_depth = self.depth_net(depth_prob)
