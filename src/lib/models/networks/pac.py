...
class PacConv2d(_PacConvNd):
    ...
    def forward(self, inp, kernel=None, mask=None):

        input_2d = inp['x']
        input_for_kernel = inp['depth']
        # print("PAC depth: ", torch.mean(input_for_kernel))
        output_mask = None
        if kernel is None:
            kernel, output_mask = self.compute_kernel(input_for_kernel, mask)
        # print("PAC", torch.mean(kernel), torch.std(kernel))
        ...
        # print("PAC out ", torch.mean(output), torch.std(output))
