...
DATA_PATH = '/home/tuan/RTM3Dv2/kitti_format/data/kitti/'
if nuscenes:
...
SPLITS = ['train1']
import _init_paths
from utils.ddd_utils import compute_box_3d, project_to_image, project_to_image3, alpha2rot_y, compute_contact_points
...
for SPLIT in SPLITS:
    image_set_path = os.path.join(DATA_PATH, "image_lr")
    ann_dir = os.path.join(DATA_PATH, "label_lr/")
    calib_dir = os.path.join(DATA_PATH, "calib_lr/")
    splits = ['trainval_lr']
    if nuscenes:
            ...
            for ann_ind, txt in enumerate(anns):
                ...
                if tmp[0] in det_cats:
                    image = cv2.imread(os.path.join(
                        image_set_path, image_info['file_name']))
                    ...
                    virtual_kps_3d = compute_contact_points(
                        dim, location, rotation_y)
                    virtual_kps_2d, _, _ = project_to_image(
                        virtual_kps_3d, calib, image.shape)
                    virtual_kps_2d = np.reshape(virtual_kps_2d, (1, 3))
                    box_2d_as_point = np.concatenate(
                        (box_2d_as_point, virtual_kps_2d), axis=1)
                    ...
                    alpha = rotation_y - \
                        math.atan2(pts_center[0, 0] - calib[0, 2], calib[0, 0])
